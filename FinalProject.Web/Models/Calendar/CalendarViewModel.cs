        ...
        {                  
            var transformed = Calendars.Select(c =>
                new  { 
                    ...
                    userId = c.UserId,
                    countyId = c.CountyId,
                    title = (c.UserId == userId || isAdmin) ? c.Title  : "",              // only display title if user owns event or isAuth
                    location =  (c.UserId == userId || isAdmin) ? c.Location : "",  // only display location if user owns event or isAuth
